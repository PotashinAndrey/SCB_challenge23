...
export const userInputHandler = function userInputHandler<T>(source: Store<T>, name?: string): Event<T> {
  return debounce({ source, timeout: TYPING_REQUEST_COOLDOWN, name });
};

/** Изменить значение поля и вызвать запуск обработчиков этого */
export const changeFieldForm = function changeFieldForm<T, F = string>(form: Form<T>): Event<{ field: string, value: F }> {
  const event = createEvent<{ field: string, value: F }>();

  sample({
    clock: event,
    source: form.$values,
    filter: (values, { field, value }) => new ObjectPath(field).getObjectValue(values) !== value,
...
/** Сбросить поле */
export const resetFieldState = function resetFieldState<T>(form: Form<T>): Event<{ field: string }> { // (form: Form, field: string): void => {
