...
type PopupBehaviourEvents<S = void> = { open: Event<S>, close: Event<void>, setPopupData: Event<any> };
type PopupBehaviourUnits<S = void> = PopupBehaviourEvents<S> & { $visible: Store<boolean>, $popupData: Store<any> };
type PopupBehaviourShape<S = void> = PopupBehaviourEvents<S> & { visible: Store<boolean>, popupData: Store<any> };
...
function factoryPopupBehaviour<S = void>(initial = false): PopupBehaviour<S> {
  ...
  return { $visible, $popupData, open, close, setPopupData, "@@unitShape": unitShape };
}
