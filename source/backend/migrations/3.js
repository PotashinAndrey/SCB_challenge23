...
/**
 ...
 export default async function migration(client, db) {
  ...
  await db.createField("company.departments", "company", "uuid", client);
  await db.createRelation({
    source: "company.companies.id",
    target: "company.departments.company"
  }, client);
  ...
  await db.createField("company.employes", "department", "uuid", client);
  await db.createRelation({
    source: "company.departments.id",
    target: "company.employes.department"
  }, client);
